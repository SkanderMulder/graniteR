...
tibble(text = "Test") |> granite_embed(text_col = text)
```
...
predictions <- granite_predict(classifier, new_data, text_col = text, type = "class")
print(predictions)

# Get probability scores
probabilities <- granite_predict(classifier, new_data, text_col = text, type = "prob")
print(probabilities)
```
...
embeddings <- data |>
  granite_embed(text_col = text, model = custom_model)
```
...
classifier <- granite_classifier(num_labels = 3) |>
  granite_train(train_data, text_col = text, label_col = category, epochs = 3)
```
