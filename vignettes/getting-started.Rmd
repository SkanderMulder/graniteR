...
graniteR requires Python dependencies to function. The package uses **UV** for fast installation (10-100x faster than pip).
...
tibble(text = "Test") |> embed(text_col = text)
```
...
The `embed()` function adds embedding columns (emb_1, emb_2, ..., emb_768) to your data.
...
predictions <- predict(classifier, new_data, text_col = text, type = "class")
print(predictions)

# Get probability scores
probabilities <- predict(classifier, new_data, text_col = text, type = "prob")
print(probabilities)
```
...
classifier <- classifier(num_labels = 3, device = "cuda")
```
...
embeddings <- data |>
  embed(text_col = text, model = custom_model)
```
...
classifier <- classifier(num_labels = 3) |>
  train(train_data, text_col = text, label_col = category, epochs = 3)
```
