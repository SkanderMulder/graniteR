...
# Predict
test <- malicious_prompts_sample |> slice_sample(n = 1000)
...
# Split data
set.seed(42)
n <- nrow(malicious_prompts_full)
train_idx <- sample(n, size = floor(0.8 * n))
train_data <- malicious_prompts_full[train_idx, ]
test_data <- malicious_prompts_full[-train_idx, ]

# Train classifier
clf <- classifier(num_labels = 2, freeze_backbone = TRUE)
clf <- train(clf, train_data, text, label, epochs = 3, batch_size = 8)

# Save model
save_classifier(clf, file = "models/malicious_prompts_standard")
```
...
clf <- load_classifier("models/malicious_prompts_standard")
