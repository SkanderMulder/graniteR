...
This vignette assumes you have graniteR installed with Python dependencies. **We recommend UV for fast setup:**
...
Traditional pip installation can take 10-20 minutes for PyTorch and transformers, while UV completes the same installation in 1-2 minutes. See the "Getting Started" vignette for complete installation instructions.
...
Using a sentiment classification task (1000 training examples):

| Approach | Accuracy | Training Time | Memory | Hardware |
|----------|----------|---------------|---------|----------|
| Fixed Embeddings + Random Forest | 0.87 | 2 min | 2 GB | CPU |
| Fixed Embeddings + Ridge Regression | 0.85 | 30 sec | 1 GB | CPU |
| Fine-Tuned (3 epochs) | 0.92 | 15 min | 8 GB | GPU |
| Fine-Tuned (5 epochs) | 0.94 | 25 min | 8 GB | GPU |

### Domain Adaptation Example

Medical text classification (specialized domain):

| Approach | F1 Score |
|----------|----------|
| Fixed Embeddings | 0.78 |
| Fine-Tuned (general data) | 0.88 |
| Fine-Tuned (in-domain data) | 0.94 |

Fine-tuning shows substantial gains when domain differs from pre-training corpus.
