...
## The Solution: S3 Methods with saveRDS() and load_classifier()

### Basic Usage (Just use saveRDS!)
...
# Save it - S3 method handles everything automatically!
saveRDS(clf, "models/emotion_clf")
...
**Key Point**: Just use standard R functions! The S3 methods handle PyTorch weights automatically.

### What Gets Saved

The `saveRDS.granite_classifier()` S3 method saves two files:
...
# Save the trained model - just use saveRDS!
saveRDS(clf, "my_models/emotion_detector_v1")
```

Output:
```
✔ Saved model to my_models
ℹ Weights: emotion_detector_v1_weights.pt
ℹ Config: emotion_detector_v1_config.rds
```
...
```
models/
├── emotion_frozen_weights.pt
├── emotion_frozen_config.rds
├── emotion_finetuned_weights.pt
├── emotion_finetuned_config.rds
├── emotion_moe_weights.pt
└── emotion_moe_config.rds
```
...
models/
├── production/
│   └── emotion_v2_weights.pt
├── staging/
│   └── emotion_v3_weights.pt
└── experiments/
    ├── exp1_frozen_weights.pt
    └── exp2_moe_weights.pt
```
...
- ✅ Use `save_classifier()` and `load_classifier()` for model persistence
- ❌ Don't use base R `save()` and `load()` - they don't work with Python objects
- 📁 Models are saved as two files: weights (.pt) and config (.rds)
- 🔄 Models can be loaded in new sessions and used immediately
- 💾 File sizes: 5-10 MB (frozen) to 500-600 MB (fine-tuned/MoE)
- ⚡ Load times: < 1 second (frozen) to 10-15 seconds (large models)
