test_that("classifier creates a valid classifier object", {
  ...
  expect_s3_class(classifier_explicit, "classifier")
  ...
  classifier_inferred <- classifier(data = data, label_col = label)
  expect_s3_class(classifier_inferred, "classifier")
...
test_that("train runs and updates the classifier", {
  ...
  expect_s3_class(trained_classifier, "classifier")
})
...
test_that("predict returns predictions", {
  ...
  classifier <- classifier(num_labels = 2) |>
    train(train_data, text, label, epochs = 1, verbose = FALSE)
  ...
  preds_class <- predict(classifier, new_data, text, type = "class")
  ...
  preds_prob <- predict(classifier, new_data, text, type = "prob")
