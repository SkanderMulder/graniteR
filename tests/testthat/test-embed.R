...
test_that("embed accepts custom model and tokenizer", {
  ...
  data <- tibble::tibble(text = c("Hello", "World"))
  result <- embed(data, text, model = model, tokenizer = tokenizer)
...
test_that("embed handles CUDA device", {
  skip_if_no_python_or_modules()
  skip_on_cran()
...
test_that("embed handles multiple batches", {
  skip_if_no_python_or_modules()
  skip_on_cran()
  
  data <- tibble::tibble(text = rep("test", 15))
  result <- embed(data, text, batch_size = 5)
